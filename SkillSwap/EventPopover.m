...
#define MENU_POINTER_RECT       CGRectMake(fabs( ( double )[ [ UIScreen mainScreen ] bounds ].size.width)-40, frame.origin.y, 23, 11)
...
@implementation EventPopover
@synthesize menuPopoverDelegate;
@synthesize menuItems;
@synthesize menuIsSelected;
@synthesize containerButton;

- (id)initWithFrame:(CGRect)frame menuItems:(NSArray *)aMenuItems menuIsSelected:(NSMutableArray *)aMenuIsSelected;
{
    ...
    if (self)
    {
        ...
        self.menuIsSelected = aMenuIsSelected;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        if ([self.menuIsSelected[indexPath.row] isEqualToString:@"Y"]) {
            [cell.textLabel setTextColor:[UIColor colorWithRed:232/255.0 green:51/255.0 blue:102/255.0 alpha:1]];
        } else {
